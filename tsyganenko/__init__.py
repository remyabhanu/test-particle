...
|   Written by Sebastien 2012-10
        ...
        assert (not None is [lat, lon, rho]) or filename, 'You must provide either (lat, lon, rho) or a filename to read from'
        if not None is [lat, lon, rho]: 
            self.lat = lat
            self.lon = lon
            self.rho = rho
            self.coords = coords
        ...
        for ip in range(len(lat)):
            ...
            for mapto in maptoL:
                ...
                if mapto == -1:
                    ...
                    self.xTrace[ip,int(self.l[ip]):int(self.l[ip])+l] = xarr[0:l]
                    self.yTrace[ip,int(self.l[ip]):int(self.l[ip])+l] = yarr[0:l]
                    self.zTrace[ip,int(self.l[ip]):int(self.l[ip])+l] = zarr[0:l]
                self.l[ip] += l

        # Resize trace output to more minimum possible length
        self.xTrace = self.xTrace[:,0:int(self.l.max())]
        self.yTrace = self.yTrace[:,0:int(self.l.max())]
        self.zTrace = self.zTrace[:,0:int(self.l.max())]
...
|   Written by Sebastien 2012-10
        ...
        for ip in inds:
            ...
            if proj[0] == 'x':
                xx = self.xTrace[ip,0:int(self.l[ip])]
                xpt = self.xGsw[ip]
                ax.set_xlabel(r'$X_{GSW}$')
                xdir = [1,0,0]
            elif proj[0] == 'y':
                xx = self.yTrace[ip,0:int(self.l[ip])]
                xpt = self.yGsw[ip]
                ax.set_xlabel(r'$Y_{GSW}$')
                xdir = [0,1,0]
            elif proj[0] == 'z':
                xx = self.zTrace[ip,0:int(self.l[ip])]
                xpt = self.zGsw[ip]
                ax.set_xlabel(r'$Z_{GSW}$')
                xdir = [0,0,1]
            if proj[1] == 'x':
                yy = self.xTrace[ip,0:int(self.l[ip])]
                ypt = self.xGsw[ip]
                ax.set_ylabel(r'$X_{GSW}$')
                ydir = [1,0,0]
            elif proj[1] == 'y':
                yy = self.yTrace[ip,0:int(self.l[ip])]
                ypt = self.yGsw[ip]
                ax.set_ylabel(r'$Y_{GSW}$')
                ydir = [0,1,0]
            elif proj[1] == 'z':
                yy = self.zTrace[ip,0:int(self.l[ip])]
                ypt = self.zGsw[ip]
            ...
            if 'x' not in proj: 
                ...
                indMask = sign*self.xTrace[ip,0:int(self.l[ip])] < 0
            if 'y' not in proj: 
                zz = sign*self.yGsw[ip]
                indMask = sign*self.yTrace[ip,0:int(self.l[ip])] < 0
            if 'z' not in proj: 
                zz = sign*self.zGsw[ip]
                indMask = sign*self.zTrace[ip,0:int(self.l[ip])] < 0
            # Plot
...
|   Written by Sebastien 2012-10
        ...
        # Then plot the traced field line
        for ip in inds:
            ax.plot3D(  self.xTrace[ip,0:int(self.l[ip])],
                        self.yTrace[ip,0:int(self.l[ip])],
                        self.zTrace[ip,0:int(self.l[ip])], 
