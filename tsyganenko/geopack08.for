...
C
Cf2py intent(in) J
Cf2py intent(in,out) :: XSM,YSM,ZSM,XGSW,YGSW,ZGSW
...
!            CALL T01_01(IOPT,PARMOD,PSI,X,Y,Z,BXGSW,BYGSW,BZGSW)
      ELSE IF (EXNAME.eq.'T04_s') THEN
            CALL T04_s(IOPT,PARMOD,PSI,X,Y,Z,BXGSW,BYGSW,BZGSW)
      ELSE IF (EXNAME.eq.'zeroB') THEN
            BXGSW = 0
            BYGSW = 0
            BZGSW = 0
      ELSE 
      		PRINT *, 'wrong model B field name'
      ENDIF
...
C=====================================================================================
C
      SUBROUTINE TSYMOD (X,Y,Z,BX,BY,BZ,BMAG,IOPT,PARMOD,
     * PSI_TILT,setPSI,EXNAME,INNAME)
C  Modification March 14,2018  German Farinas
C  Added the option of explicitly set PSI or use the one calculated by recalc routine
C  If setPSI > 0 use PSI_TILT value, if setPSI < 0 use PSI as recalc_08
C
C  CALCULATES THE COMPONENTS OF THE RIGHT HAND SIDE VECTOR IN THE GEOMAGNETIC FIELD
C    LINE EQUATION  (a subsidiary subroutine for the subroutine STEP_08)
C
...
Cf2py intent(in) ::  PSI_TILT,setPSI
Cf2py intent(out) ::  BX, BY, BZ,BMAG
C
      DIMENSION PARMOD(10)
C
C     EXNAME AND INNAME ARE NAMES OF SUBROUTINES FOR THE EXTERNAL AND INTERNAL
C     PARTS OF THE TOTAL FIELD, E.G., T96_01 AND IGRF_GSW_08
C
      COMMON /GEOPACK1/ A(12),BB(2),PSI,CC(18)
C       EXTERNAL EXNAME,INNAME
      CHARACTER EXNAME*(*),INNAME*(*)
...
!       print*, '-------IN--',X,Y,Z,HXGSW,HYGSW,HZGSW,BXGSW,BYGSW,BZGSW
      IF (EXNAME.eq.'T96_01') THEN
            CALL T96_01(IOPT,PARMOD,PSIt,X,Y,Z,BXGSW,BYGSW,BZGSW)
!      ELSE IF (EXNAME.eq.'T01_01') THEN
!            CALL T01_01(IOPT,PARMOD,PSIt,X,Y,Z,BXGSW,BYGSW,BZGSW)
      ELSE IF (EXNAME.eq.'T04_s') THEN
            CALL T04_s(IOPT,PARMOD,PSIt,X,Y,Z,BXGSW,BYGSW,BZGSW)
	  ELSE IF (EXNAME.eq.'zeroB') THEN
            BXGSW = 0
            BYGSW = 0
            BZGSW = 0
...
C
!===========================================================================================
      SUBROUTINE TRACE1_08 (XIsm,YIsm,ZIsm,DIR,DSMAX,ERR,RLIM,R0,IOPT,
     * PARMOD,EXNAME,INNAME,XFsm,YFsm,ZFsm,XXsm,YYsm,ZZsm,L,LMAX,Liout)
!===========================================================================================
!   Warning: This routine is under testing because is not working properly
...
!
      ...
      DIMENSION XX(LMAX),YY(LMAX),ZZ(LMAX), PARMOD(10)
      DIMENSION XXsm(LMAX),YYsm(LMAX),ZZsm(LMAX)
      COMMON /GEOPACK1/ AA(26),DD,BB(8)
      CHARACTER EXNAME*(*),INNAME*(*)
Cf2py intent(in) ::  XIsm, YIsm, ZIsm
Cf2py intent(in) ::  DIR, R0, RLIM, IOPT, PARMOD
Cf2py intent(in) ::  EXNAME, INNAME
Cf2py integer intent(in) ::  LMAX
Cf2py intent(out) :: XFsm,YFsm,ZFsm
Cf2py intent(out),depend(LMAX) :: XXsm,YYsm,ZZsm
Cf2py intent(out) :: L
Cf2py intent(out) :: iout
!
      iout=0
!     ERR=0.0001
      L=0
      DS=0.5*DIR
      Zrsm=Zism
      call SMGSW_08(Xism,Yism,Zism,XI,YI,ZI,i_one)
!      PRINT *, '  Xism ',Xism
!      PRINT *, '  Yism ',Yism
!      PRINT *, '  Zism ',Zism
!      PRINT *, '  XI ',XI
...
!     AD is defined in order to initialize the value of RR (radial distance at previous step):
      ...
      RR=SQRT(X**2+Y**2+Z**2)+AD
  1   L=L+1
      IF(L.GT.(LMAX-1)) GOTO 7
      XX(L)=X
      YY(L)=Y
      ZZ(L)=Z
      call SMGSW_08(Xsm,Ysm,Zsm,X,Y,Z,m_one)
      XXsm(L)=Xsm
      YYsm(L)=Ysm
      ZZsm(L)=Zsm
      RYZ=Y**2+Z**2
...
!
 ...
 99   R1=-Zsm/(Zrsm-Zsm)
      ...
      Ysm=Ysm-(Ysm-Yrsm)*R1
      Zsm=Zsm-(Zsm-Zrsm)*R1
      call SMGSW_08(Xsm,Ysm,Zsm,X,Y,Z,i_one)
      goto 8
